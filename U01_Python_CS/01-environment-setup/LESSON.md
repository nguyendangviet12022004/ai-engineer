...
> **Tài liệu tham khảo:** MIT Missing Semester (6.NULL) L1-2 · Python Packaging User Guide · Real Python: Virtual Environments
...
1. Cài được Python 3.11+ và quản lý được **nhiều phiên bản Python** trên cùng 1 máy bằng `pyenv`.
2. Hiểu và dùng thành thạo **`uv`** để quản lý dependency (thay cho pip/venv/poetry rời rạc).
...
- Một cách quản lý **nhiều phiên bản Python** (3.10, 3.11, 3.12...) song song → dùng **pyenv**.
- Một cách cô lập dependency **theo từng dự án** → dùng **virtual environment**.

### 2.2. pyenv — quản lý phiên bản Python

```bash
# Cài pyenv (Linux/macOS, xem pyenv-installer cho chi tiết)
...
# Đặt làm phiên bản mặc định toàn cục
pyenv global 3.11.9

# Đặt phiên bản riêng cho 1 thư mục dự án (tạo file .python-version)
cd my-project && pyenv local 3.11.9
```

`pyenv` hoạt động bằng cách chèn "shim" vào PATH — khi bạn gõ `python`, shim này sẽ đọc file `.python-version` (theo thứ tự: thư mục hiện tại → thư mục cha → global) để quyết định gọi bản Python nào.
...
| `poetry` | Không (dựa vào pyenv) | pip-compatible | Chậm | Dự án Python thuần, cần publish package |
| **`uv`** | Có (tích hợp) | pip-compatible, viết bằng Rust | **Rất nhanh (10-100x pip)** | **Mặc định cho mọi dự án trong lộ trình này** |

**Quyết định của lộ trình: dùng `uv` làm công cụ mặc định.** Lý do: `uv` giải quyết dependency graph bằng Rust nên nhanh hơn pip 10-100 lần, tích hợp luôn venv + version resolution + lock file, và đang trở thành chuẩn mới trong cộng đồng Python (2024-2026).
...
**Bước 4 — Ghi lại phiên bản numpy hiện tại (để đối chiếu sau):**
...
**Bước 7 — Đảm bảo `.venv` không vào git:**
...
Bạn đã có:
- ✅ Một cách quản lý Python version (`pyenv`) và dependency (`uv`) chuyên nghiệp, nhanh, tái lập được.
